...
# AUTO_PAY = False   → bot clicks PAY automatically (use only with a real card)
AUTO_PAY = False    # ← change to False if you just want to review before paying
...
async def run_in_browser(slot):
    ...
    try:
        ...
        # Helper: fill an Angular input using execCommand (reliable with Angular)
        async def fill_field(selector, value):
            el = await tab.query_selector(selector)
            if not el:
                log(f"  ⚠️  Element not found: {selector}")
                return
            # Click to focus the Angular Material form field
            await el.click()
            await tab.sleep(0.2)
            # Select all + insert text via execCommand (triggers Angular zone)
            await tab.evaluate(f"""
                (() => {{
                    ...
                    document.execCommand('insertText', false, "{str(value)}");
        ...
        # Click GDPR checkboxes — find by label text, click the mat-checkbox
        await tab.evaluate("""
            (() => {
                // Find all mat-checkbox elements
                const checkboxes = Array.from(document.querySelectorAll('mat-checkbox'));
                for (const cb of checkboxes) {
                    const label = cb.querySelector('.mdc-label')?.innerText || cb.innerText || '';
                    // Terms checkbox (first required one)
                    if (label.includes('Norme Generali') || label.includes('Accetto le')) {
        ...
        await tab.evaluate("""
            (() => {
                ...
                for (const cb of checkboxes) {
                    const label = cb.querySelector('.mdc-label')?.innerText || cb.innerText || '';
                    if (label.includes('offerte') || label.includes('ricevere informazioni')) {
                        const input = cb.querySelector('input[type="checkbox"]');
                        if (input && !input.checked) {
                            cb.click();
                            console.log('Clicked privacy checkbox');
                        }
                    }
                }
                ...
                for (const inp of inputs) {
                    if (!inp.checked && inp.offsetParent !== null) {
                        const matCb = inp.closest('mat-checkbox');
                        if (matCb) matCb.click();
                        else inp.click();
                    }
                }
            })()
        ...
        """)
        log(f"  Checkboxes after: {cb_status}")
        await tab.sleep(3)  # Let Angular validate

        # Verify form validity
        invalid_fields = await tab.evaluate("""
            (() => {
                const invalid = Array.from(document.querySelectorAll('.ng-invalid[data-cy], .ng-invalid input, .ng-invalid mat-select'))
                    .map(el => el.getAttribute('data-cy') || el.id || el.name || el.placeholder || el.tagName)
                    .filter(Boolean);
                return invalid.slice(0, 15);
            })()
        """)
        if invalid_fields:
            log(f"  ⚠️  {len(invalid_fields)} invalid fields: {invalid_fields[:5]}")
        else:
