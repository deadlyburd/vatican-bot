...
# Buyer data — overridden by CRM participants when available
BUYER = {"surname": "skear", "name": "abiilesh", "email": "your-email@example.com", "phone": "your-phone-number", "city": "roma"}
PARTICIPANTS = []  # Set from CRM: [{"first_name":"Mario","last_name":"Rossi","type":"Adult"},...]

def set_crm_data(buyer=None, participants=None):
    """Called before book() to use real CRM names."""
    global BUYER, PARTICIPANTS
...
async def book(slot, retry_count=0):
    ...
    browser = await uc.start(
        ...
        lang="it-IT",
        no_sandbox=True,
        window_size=(701, 628),  # From working recording
        browser_args=[
    ...
    try:
        ...
        if 'checkout' not in str(cur):
            log("ERROR: Not on checkout!")
            if retry_count < MAX_RETRIES - 1:
                log(f"  🔄 Retry {retry_count+2}/{MAX_RETRIES}...")
                await asyncio.sleep(3)
                return await book(slot, retry_count + 1)
        ...
        # Participants — from CRM data, fallback to hardcoded
        crm_participants = PARTICIPANTS if PARTICIPANTS else [
            {"last_name":"Sekar","first_name":"Abiilesh","type":"Adult"},
            {"last_name":"Bianchi","first_name":"Sofia","type":"Adult"},
        ]
        for i, p in enumerate(crm_participants[:v]):  # One row per person
            if i > 0:
                await tab.evaluate(f"""(function(){{
                    var el = document.querySelector('#participantElement_{i} div.tw-flex-grow > div > div.tw-flex');
                    if (!el) el = document.querySelector('#participantElement_{i} div.tw-flex-grow > div');
                    if (el) el.click();
                }})()""")
                await tab.sleep(0.5)
            last = p.get("last_name", p.get("last", f"Cognome{i+1}"))
            first = p.get("first_name", p.get("first", f"Nome{i+1}"))
            await fill_input(f"#participantSurname_{i}", last)
            await fill_input(f"#participantName_{i}", first)
        ...
        # Wait for Angular to render checkboxes
        await asyncio.sleep(1)

        # ── GDPR CHECKBOX — find by aria text "Accetto le" ──
        gdpr_result = await tab.evaluate("""(()=>{
            // Search ALL checkbox inputs for the GDPR one
            var inputs = document.querySelectorAll('input[type=\"checkbox\"]');
            for (var i=0; i<inputs.length; i++) {
                var inp = inputs[i];
                if (!inp.offsetParent || inp.checked) continue;
                // Check aria-label on the input or parent mat-checkbox text
                var parent = inp.closest('mat-checkbox');
                var text = parent ? (parent.innerText || parent.textContent || '') : '';
                var aria = inp.getAttribute('aria-label') || '';
                if (text.indexOf('Accetto le')>-1 || text.indexOf('Norme Generali')>-1 || aria.indexOf('Accetto')>-1) {
                    parent ? parent.click() : inp.click();
                    return 'gdpr-by-text';
                }
            }
            // Fallback: click first visible unchecked checkbox (likely GDPR)
            for (var j=0; j<inputs.length; j++) {
                if (!inputs[j].checked && inputs[j].offsetParent) {
                    var p = inputs[j].closest('mat-checkbox');
                    (p || inputs[j]).click();
                    return 'gdpr-fallback-idx-'+j;
                }
            }
            return 'gdpr-not-found';
        })()""")
        log(f"  GDPR: {gdpr_result}")
        await asyncio.sleep(1.5)

        # Close terms dialog
        await tab.evaluate("""(()=>{
            var close = document.querySelector('[data-cy=\"purchase-rules-close-btn\"]');
            if (close) { close.click(); return; }
            var span = document.querySelector('div.cdk-overlay-container span.mat-focus-indicator');
            if (span) span.click();
        })()""")
        await asyncio.sleep(1)

        # ── REDUCED TICKET CHECKBOX — find by aria "ridotto" ──
        reduced_result = await tab.evaluate("""(()=>{
            var inputs = document.querySelectorAll('input[type=\"checkbox\"]');
            for (var i=0; i<inputs.length; i++) {
                var inp = inputs[i];
                if (!inp.offsetParent || inp.checked) continue;
                ...
                var text = parent ? (parent.innerText || parent.textContent || '') : '';
                var aria = inp.getAttribute('aria-label') || '';
                // The reduced ticket has a very long Italian aria text
                if (text.indexOf('ridotto')>-1 || text.indexOf('ragazzi')>-1 ||
                    aria.indexOf('ridotto')>-1 || aria.indexOf('ragazzi')>-1) {
                    parent ? parent.click() : inp.click();
                    return 'reduced-by-text';
                }
            }
            // Fallback: click any remaining unchecked checkbox after GDPR
            for (var j=0; j<inputs.length; j++) {
                if (!inputs[j].checked && inputs[j].offsetParent) {
                    var t = (inputs[j].closest('mat-checkbox')?.innerText || '');
                    if (t.indexOf('Norme')==-1) {  // Skip GDPR
                        var p = inputs[j].closest('mat-checkbox');
                        (p || inputs[j]).click();
                        return 'reduced-fallback-idx-'+j;
                    }
                }
            }
            return 'reduced-not-found';
        })()""")
        log(f"  Reduced: {reduced_result}")
        await asyncio.sleep(1)
        ...
        if retry_count < MAX_RETRIES - 1:
            log(f"  🔄 Retry {retry_count+2}/{MAX_RETRIES} in 3s...")
            await asyncio.sleep(3)
            return await book(slot, retry_count + 1)
        return {'success': False, 'slot': slot, 'error': 'no epay after retries'}

    except Exception as e:
        log(f"Error: {e}"); import traceback; traceback.print_exc()
        if retry_count < MAX_RETRIES - 1:
            log(f"  🔄 Retry after error {retry_count+2}/{MAX_RETRIES}...")
            await asyncio.sleep(3)
            return await book(slot, retry_count + 1)
